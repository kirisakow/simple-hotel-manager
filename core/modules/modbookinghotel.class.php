 ...
 */
class modbookinghotel extends DolibarrModules
{
	/**
...
// Paramétrage
	...
	public function init($options='')
	{
		...
		if($resql){
	        ...
	        dolibarr_set_const($this->db, 'COMPTE_CAISSE_HOTEL', $idcompte, 'int', 0, '', 0);
		}

		$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'societe (rowid, nom, name_alias, fournisseur, entity, datec, fk_user_creat, status, fk_pays) VALUES (2, "Boutique Hôtel", "Boutique Hôtel", 1, '.$conf->entity.', "'.$this->db->idate(dol_now()).'", 1, 1, 12)';
		$resql = $this->db->query($sql);
		if($resql){
			$idfourn=$this->db->last_insert_id(MAIN_DB_PREFIX.'societe');
	        dolibarr_set_const($this->db, 'FOURNISSEUR_BOUTIQUE_HOTEL', $idfourn, 'int', 0, '', 0);
		}
		...
		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_FACTURE_DIRECT',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_FACTURE_DIRECT',0 ,'yesno',0,'',$conf->entity);
		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_DASHBOARD_AVANCE_THREEDAYS',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_DASHBOARD_AVANCE_THREEDAYS',3 ,'others',0,'',$conf->entity);

		dolibarr_set_const($this->db, "MAIN_MODULE_BOOKINGHOTEL_TABS_0", "", 'chaine', 0, '',$conf->entity);
		dolibarr_set_const($this->db, "MAIN_MODULE_BOOKINGHOTEL_TABS_1","", 'chaine', 0, '',$conf->entity);
		dolibarr_set_const($this->db, "MAIN_MODULE_BOOKINGHOTEL_TABS_2", "", 'chaine', 0, '',$conf->entity);

		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_CREATE_DEFAULT_TIME_START',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_CREATE_DEFAULT_TIME_START','08-00' ,'others',0,'',$conf->entity);
		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_CREATE_DEFAULT_TIME_END',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_CREATE_DEFAULT_TIME_END','18-00' ,'others',0,'',$conf->entity);
		
		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_ACTIVATE_INVOICES',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_ACTIVATE_INVOICES', 1,'chaine',0,'',$conf->entity);
		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_ACTIVATE_PROPALS',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_ACTIVATE_PROPALS', 1,'chaine',0,'',$conf->entity);
		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_ACTIVATE_ORDERS',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_ACTIVATE_ORDERS', 1,'chaine',0,'',$conf->entity);

		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_ACTIVATE_RESERVATIONS',$conf->entity))
			dolibarr_set_const($this->db,'BOOKINGHOTEL_ACTIVATE_RESERVATIONS', 1,'chaine',0,'',$conf->entity);

		if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_FIRST_INSTALLATION',$conf->entity)){
			...
			if ($result > 0){
				dolibarr_set_const($this->db,'BOOKINGHOTEL_GESTION_CATEGORIES_A_RESERVER', $result,'chaine',0,'',$conf->entity);
				dolibarr_set_const($this->db,'BOOKINGHOTEL_FIRST_INSTALLATION', 1,'chaine',0,'',$conf->entity);
			}
		}
