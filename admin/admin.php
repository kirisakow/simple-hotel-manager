...
if ($action == 'set_categories_reservee') {
	...
	$res1 = dolibarr_set_const($db, "BOOKINGHOTEL_GESTION_CATEGORIES_A_RESERVER", $chambres, 'tbnotb', 0, '', $conf->entity);
	if (! $res1 > 0)	$error ++;
...
print '<form id="col2-form" method="post" action="admin.php">';
  ...
  // Bank
  if(dolibarr_get_const($db, 'CATEGORIES_PRODHOTEL', 0)){
      $CATEGORIES_PRODHOTEL = dolibarr_get_const($db, 'CATEGORIES_PRODHOTEL', 0);
...
print '<tr>';
    $FOURNISSEUR_BOUTIQUE_HOTEL = dolibarr_get_const($db, 'FOURNISSEUR_BOUTIQUE_HOTEL', 0);
...
print '<tr>';
    ...
    print '<td>';
	    $COMPTE_CAISSE_HOTEL = dolibarr_get_const($db, 'COMPTE_CAISSE_HOTEL', 0);
