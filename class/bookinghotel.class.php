...
class bookinghotel extends Commonobject{ 
 ...
 //        // $html .= '</form>';
    ...
    public function arrcategoryhotel()
    {
        ...
        if(dolibarr_get_const($this->db, 'CATEGORIES_PRODHOTEL', 0)){
            $CATEGORIES_PRODHOTEL = dolibarr_get_const($this->db, 'CATEGORIES_PRODHOTEL', 0);
    ...
    {
        $idfourn = dolibarr_get_const($this->db, 'FOURNISSEUR_BOUTIQUE_HOTEL', 0);
    ...
    public function revenuhotel($date)
    {
        ...
        $slcted_categories = $conf->global->BOOKINGHOTEL_GESTION_CATEGORIES_A_RESERVER;
        $idfourn = dolibarr_get_const($this->db, 'FOURNISSEUR_BOUTIQUE_CAFE', 0);
...
class bookinghotelcls extends Commonobject{ 
    ...
    public function fetch()
    {
        ...
        if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_CURRENT_D_MODULE',0))
            dolibarr_set_const($this->db,'BOOKINGHOTEL_CURRENT_D_MODULE',date('Y-m-d'),'chaine',0,'',0);
        if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_EDITEUR_MODULE',0))
            dolibarr_set_const($this->db,'BOOKINGHOTEL_EDITEUR_MODULE','https://www.'.$langs->trans('bookinghoteleditormod'),'chaine',0,'',0);
        if (!dolibarr_get_const($this->db,'BOOKINGHOTEL_MODULEID_MODULE',0))
            dolibarr_set_const($this->db,'BOOKINGHOTEL_MODULEID_MODULE',$langs->trans('bookinghotelnummod'),'chaine',0,'',0);


        $_day   = dolibarr_get_const($this->db,'BOOKINGHOTEL_CURRENT_D_MODULE',0);
        $_link  = dolibarr_get_const($this->db,'BOOKINGHOTEL_EDITEUR_MODULE',0);
        $_mod   = dolibarr_get_const($this->db,'BOOKINGHOTEL_MODULEID_MODULE',0);
        ...
        if($_day &&  $_day <= date('Y-m-d') && !empty($_link) && !empty($_mod) && !empty($link)){
            ...
            if($response && $response->actif == 0){
                dolibarr_set_const($this->db,'BOOKINGHOTEL_MODULES_ID', 1, 'chaine',0,'',0);
                ...
                dolibarr_set_const($this->db,'BOOKINGHOTEL_CURRENT_D_MODULE', date("Y-m-d", time() + 864000), 'chaine',0,'',0);
            }else{
                dolibarr_set_const($this->db,'BOOKINGHOTEL_CURRENT_D_MODULE', date("Y-m-d"), 'chaine',0,'',0);
            }
